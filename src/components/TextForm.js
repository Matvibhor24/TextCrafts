...
import './TextForm.css'
import icon from '../assets/copy_icon.png';
        ...
        <>
            <div className="comp_container">
                <div className={`heading-tab text-${props.mode === 'light' ? 'black' : 'white'}`}>
                    <h5>{props.heading}</h5>
                    {/* <FunctionButtonBar/> */}
                    <div className={`btn-group bg-${props.mode === 'light' ? 'success-subtle' : 'secondary'} btn-outline-success shadow-lg rounded`} align="right" role="group" aria-label="Basic example" >
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={onUpClick} >To Uppercase</button>
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={onLoClick}>To lowercase</button>
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={convertToTitleCase}>To Titlecase</button>
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={onClearHandler}>Clear Text</button>
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={onRemoveSpaceHandler}>Remove Extra spaces</button>
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={removeSpecialCharacters}>Remove Special Characters</button>
                        <button type="button" disabled={text === ""} className={`btn text-${props.mode === 'light' ? 'success-subtle' : 'white'}`} onClick={reverseText}>Reverse Text</button>
                    </div>
                </div>
